...
manifest_id="frequency_color_circle__train_5932a18b5983"
...
short_gpu=4
long_gpu=5
short_config="configs/pendulum/Train-frequency_color_circle-50k-short.yaml"
long_config="configs/pendulum/Train-frequency_color_circle-50k-long.yaml"

mkdir -p "$log_root"
exec 9>"${log_root}/pipeline.lock"
if ! flock -n 9; then
  echo "Another frequency_color_circle 50k pipeline is already active."
  exit 1
fi
...
for history in short long; do
  ...
  checkpoint_dir="${run_root}/${history}/ckpt/frequency_color_circle-50k-${history}"
...
if [[ "$short_ecc_baseline" != "1" ]]; then
  echo "GPU ${short_gpu} ECC changed from the accepted baseline of 1: ${short_ecc_baseline}"
  exit 1
fi
if [[ "$long_ecc_baseline" != "0" ]]; then
  echo "GPU ${long_gpu} ECC changed from the accepted baseline of 0: ${long_ecc_baseline}"
  exit 1
fi
...
require_file "${run_root}/short/ckpt/frequency_color_circle-50k-short/step-50000.safetensors"
require_file "${run_root}/long/ckpt/frequency_color_circle-50k-long/step-50000.safetensors"
