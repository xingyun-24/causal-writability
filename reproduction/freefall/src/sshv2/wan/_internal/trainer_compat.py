"""Training module for the checkpoint-compatible shared Wan variant."""

import torch

from sshv2.wan._internal.sampler_compat import WanVideoPipeline
...
class CompatibilityWanTrainingModule(DiffusionTrainingModule):
    def __init__(
        self,
        ...
        if getattr(dit_config, "latent_canonical_mode", "none") == "steered":
            raise ValueError(
                "The removed steered-canonical prototype is not supported"
            )
    ...
    def forward_preprocess(self, data):
        ...
        if "motion_tube" in data:
            inputs_shared["motion_tube"] = data["motion_tube"]
        if "canonical_motion" in data:
            inputs_shared["canonical_motion"] = data["canonical_motion"]
        if "trajectory_maps" in data:
            inputs_shared["trajectory_maps"] = data["trajectory_maps"]
        if "renderer_context" in data:
            inputs_shared["renderer_context"] = data["renderer_context"]
        if "wall_geometry" in data:
            inputs_shared["wall_geometry"] = data["wall_geometry"]
        if "vae_local_condition" in data:
            inputs_shared["vae_local_condition"] = data["vae_local_condition"]
    ...
    def collate_fn(data):
        ...
        if "metadata" in data[0]:
            ...
            if isinstance(metadata, torch.Tensor):
                values=torch.stack([sample["metadata"] for sample in data])
                if values.ndim == 5: batch["trajectory_maps"] = values
                else: batch["motion_tube"] = values
            elif isinstance(metadata, dict):
                # Metadata-driven experiments may carry several aligned
                # conditions (e.g. trajectory maps plus a ball-erased static
                # context).  Preserve all tensor fields rather than silently
                # dropping a new condition in the collation path.
                for key, value in metadata.items():
                    if isinstance(value, torch.Tensor):
                        batch[key] = torch.stack([sample["metadata"][key] for sample in data])
        return batch
