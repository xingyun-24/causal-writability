#!/usr/bin/env bash
# Encode V2 samples at varied prediction starts, then train a separate run.
set -euo pipefail
...
LATENT_DIR="$ROOT/data/latents/train_short_v2_varstart"
RUN=short-v2-varstart
...
python3 "$ROOT/scripts/train_projectile.py" \
  --config "$ROOT/config/Train-short-v2-varstart.yaml" \
