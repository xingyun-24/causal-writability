#!/usr/bin/env bash
set -euo pipefail
SHARD=${1:?usage: queue_recovery_worker_remote.sh SHARD GPU}
GPU=${2:?usage: queue_recovery_worker_remote.sh SHARD GPU}
...
while true; do
  ...
  if [ "$FREE" -ge "$MIN_FREE_MB" ]; then
    exec bash "$ROOT/scripts/run_recovery_worker_remote.sh" "$SHARD" "$GPU"
  fi
