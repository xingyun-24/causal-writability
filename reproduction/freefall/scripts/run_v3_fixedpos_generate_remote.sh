...
OUT="$ROOT/data/raw-v3-fixedpos-2048"
test ! -e "$OUT/train"
mkdir -p "$ROOT/data" "$ROOT/logs"
cd "$REPO"
export PYTHONPATH="$REPO/src:$REPO/lib/diffsynth"
python3 "$ROOT/scripts/build_dataset_v2.py" --config "$ROOT/config/data-v3-fixedpos.yaml" --out "$OUT" --split train --base-seeds 1024
