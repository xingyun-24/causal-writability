#!/usr/bin/env python3
"""Roll out color*[1,g,sqrt(g)] PCA fit using an opposite-colour target state."""
...
def main() -> None:
    ...
    for count, (pair_id, pair) in enumerate(selected, 1):
        conflict = metadata[(pair_id, "conflict")]; aligned = metadata[(pair_id, "aligned")]
        conflict_eval = eval_rows[(pair_id, "conflict")]
        c_current = colour_value(conflict["color_label"]); c_target = colour_value(aligned["color_label"])
        g_current = float(conflict_eval["g_E3"]); g_target = float(pair["gravity_true"])
        feature = torch.tensor([
            c_target - c_current,
            c_target * g_target - c_current * g_current,
            c_target * np.sqrt(g_target) - c_current * np.sqrt(max(g_current, 0.0)),
        ...
        results.append({"pair_id": pair_id, "input_colour": conflict["color_label"], "target_colour": aligned["color_label"], "target_g": g_target, "current_g_E3": g_current, "predicted_scores": score.detach().cpu().tolist(), **colours, **measured})
        print(f"pairs={count}/{len(selected)}", flush=True)
    target_matched = [row for row in results if row["detected_colour_majority"] == row["target_colour"]]
    input_preserved = [row for row in results if row["detected_colour_majority"] == row["input_colour"]]
    payload = {"method": "cross-colour rollout of color*[1,g,sqrt(g)] PCA fit", "block": args.block, "pairs": len(results), "E3_accuracy": float(np.mean([row["E3_correct"] for row in results])), "E0_accuracy": float(np.mean([row["E0_correct"] for row in results])), "mean_abs_g_error": float(np.mean([row["gravity_error"] for row in results if row["gravity_error"] is not None])), "target_colour_matched_pairs": len(target_matched), "input_colour_preserved_pairs": len(input_preserved), "results": results}
    ...
    print(json.dumps({key: payload[key] for key in ("pairs", "E3_accuracy", "E0_accuracy", "mean_abs_g_error", "target_colour_matched_pairs", "input_colour_preserved_pairs")}, indent=2))
