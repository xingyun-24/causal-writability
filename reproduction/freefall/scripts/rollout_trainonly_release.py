#!/usr/bin/env python3
"""Archived train-only controller, loading its exported basis without source residuals."""
...
def main():
    ...
    p.add_argument('--limit',type=int,default=0,help='0 means all held-out receivers in this shard')
    ...
    run_spec={'controller_sha256':sha(a.controller_json),'pca_sha256':sha(a.pca_dir/'train_only_pca.npz'),
              ...
              'basis_sha256':sha(a.basis),'basis_source':'released train-only dense components'}
    ...
    basis=np.load(a.basis,mmap_mode='r')
    if basis.shape != (2,)+shape: raise ValueError(f'Wrong basis shape: {basis.shape}')
    directions=torch.tensor(np.array(basis,copy=True),dtype=torch.float32,device=device).reshape(2,-1)
    assert len(ids[train])==len(ids[heldout])==64
    assert set(ids[train]).isdisjoint(ids[heldout])
