"""Core physics, rendering, detection, and metrics for spring_shortcuts_v4.
...
class SpringDatasetConfig:
    ...
    # The default reproduces V4 exactly.  A control may exchange this
    # association without changing any physical, rendering, or optimisation
    # parameter.
    slow_color: ColorName = "red"
    fast_color: ColorName = "blue"
    ...
    def __post_init__(self) -> None:
        ...
        if {self.slow_color, self.fast_color} != {"red", "blue"}:
            raise ValueError("slow_color and fast_color must be a red/blue permutation")
...
def canonical_color_for_config(band: BandName, cfg: SpringDatasetConfig) -> ColorName:
    """Return the canonical colour under the configured band permutation."""
    return cfg.slow_color if band == "slow" else cfg.fast_color
...
def color_band_for_config(color: ColorName, cfg: SpringDatasetConfig) -> BandName:
    """Return the band implied by a colour under the configured permutation."""
    return "slow" if color == cfg.slow_color else "fast"
    ...
    *,
    ...
    for base_index in range(num_base_seeds):
        ...
        for band_index, band_name in enumerate(("slow", "fast")):
            ...
            canonical = canonical_color_for_config(band_name, cfg)
...
def audit_rows(rows: Sequence[SpringSample], cfg: SpringDatasetConfig, *, expect_conflicts: bool) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if row.variant == "aligned" and row.color_label != canonical_color_for_config(row.true_band, cfg):
            errors.append(f"noncanonical aligned row {row.sample_id}")
        if row.variant == "conflict" and row.color_label == canonical_color_for_config(row.true_band, cfg):
...
def config_to_dict(cfg: SpringDatasetConfig) -> dict[str, Any]:
    return {
        ...
        "slow_color": cfg.slow_color,
        "fast_color": cfg.fast_color,
...
def dataclass_config_from_dict(payload: dict[str, Any]) -> SpringDatasetConfig:
    ...
    return SpringDatasetConfig(
        ...
        slow_color=str(payload.get("slow_color", "red")),
        fast_color=str(payload.get("fast_color", "blue")),
    )
