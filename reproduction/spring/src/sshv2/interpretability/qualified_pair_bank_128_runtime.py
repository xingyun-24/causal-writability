"""Runtime helpers for the frozen 128-pair fast red/blue bank."""
...
def evaluate_future(
    ...
    *,
    ...
    if true_band_label == "fast":
        true_band = cfg.fast_band
    elif true_band_label == "slow":
        true_band = cfg.slow_band
    else:
        raise ValueError(f"Unknown true band: {true_band_label!r}")
...
def load_wan_pipe(*, config: Path, checkpoint: Path, steps: int, device: str, cfg: Any) -> tuple[Any, Any]:
    ...
    architecture = (
        int(train_cfg.model.dit.dim),
        int(train_cfg.model.dit.ffn_dim),
        int(train_cfg.model.dit.num_heads),
        int(train_cfg.model.dit.num_layers),
    )
    if architecture not in {
        ...
        raise AssertionError(f"Unsupported frozen DiT architecture: {architecture}")
