"""Minimal no-text Wan video pipeline for spring stage 1."""
...
from sshv2.diffsynth.models.wan_video_vae import IdentityWanVideoVAE
...
class WanVideoPipeline(BaseWanVideoPipeline):
    ...
    def from_pretrained(
        ...
        if tokenizer_config is not None:
            raise ValueError("Spring stage 1 is text-free and does not accept a tokenizer config")
        if use_usp:
            raise ValueError("USP is not used by the standalone spring stage-1 pipeline")
    ...
    def __call__(
        ...
        if cfg_scale != 1.0:
            raise ValueError("The text-free spring model requires cfg_scale=1.0")
